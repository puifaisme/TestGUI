    ...
    try:
        if token_out:
            if (
                room_text.get() == ""
                or guest_text.get() == ""
                or check_in_text.get() == ""
                or check_out_text.get() == ""
            ):
        ...
        messagebox.showerror("Login Failure", "You have to Login first !!!")
    ...
    try:
        if token_out:
            if (
                ...
                response = requests.delete(end_point, json=data, headers=headers)
                print(response.content)
